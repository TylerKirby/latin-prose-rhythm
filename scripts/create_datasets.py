...
if __name__ == '__main__':
    ...
    author_paths = ['/'.join([args.path,p]) for p in os.listdir(args.path) if p != '.DS_Store']
    ...
    for author_path in author_paths:
        ...
        print(author)
        analyze = Analyze()
        text_paths = sorted(['/'.join([author_path, p]) for p in os.listdir(author_path) if p[0] != '.'])
        ...
        rhythm_data.to_csv('../data/{}_rhythms.csv'.format(author), index=None)
        rhythm_data = pd.read_csv('../data/{}_rhythms.csv'.format(author))

        texts = sorted([text[:-4].replace('_', ' ') for text in os.listdir(author_path) if text[0] != '.'])
        ...
        artistic_rhythms = [
           ...
           'double/molossus-cretic ep res (-u---ux)', 'double trochee (-u-x)',
           'double trochee 1 res (uuu-x)', 'double trochee 1 res (-uuux)',
           'hypodochmiac (-u-ux)', 'hypodochmiac 1 res (uuu-ux)',
           'hypodochmiac 1 res (-uuuux)'
        ]
        unartistic_rhythms = [
